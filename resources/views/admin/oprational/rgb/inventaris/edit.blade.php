    ...
    <div class="p-6">
        ...
        <div class="flex mt-4">
            <span class="text-gray-400 mb-2">
                Barang yang sebelumnya ditambahkan: ( masukan lagi jika dbutuhkan ) <br>
                @foreach (explode(', ', $inventaris->deskripsi_barang) as $item)
                    <span class="text-yellow-400">{{ $item }}</span><br>
                @endforeach
            </span>
        </div>
        <form action="{{ route('inventaris.update', $inventaris->id) }}" method="POST" class="mt-4">
            @csrf
            @method('PUT')
            ...
            <div class="mt-4">
                ...
                <ul id="itemList" class="text-white">
                </ul>
            </div>
                ...
                <select name="lokasi" id="lokasi"
                    ...
                    @foreach ($data as $item)
                        <option value="{{ $item->area }}"
                            {{ $item->area == old('lokasi', $inventaris->lokasi) ? 'selected' : '' }}>
                            {{ $item->area }}
                        </option>
                    @endforeach
                </select>
